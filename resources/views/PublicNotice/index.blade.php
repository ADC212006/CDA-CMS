    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-transparent">
                <h5 class="modal-title" id="addModalLabel">Add New Public Notes</h5>
...
<script>
    $(document).ready(function() {
        var table = $('.datatables-ajax').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("public.notes.fetch") }}', // Update the API endpoint
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        return `
                <p id="action-${data}">
                 <a href="{{ env('WEBSITE_URL') . 'publicNotices' }}" target="_blank">
        ...
        $("#editForm").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: '{{ route("public.notes.update", ":id") }}'.replace(':id', id),
                    method: 'POST',
        ...
        var deleteUrl = "{{ route('public.notes.destroy', ['id' => '__id__']) }}";
        ...
        $("#addForm").validate({
            ...
            submitHandler: function(form) {
                // On form validation success, make an AJAX request
                $.ajax({
                    url: '{{route("public.notes.store")}}', // Your API endpoint
                    method: 'POST',
                    data: new FormData(form),
                    contentType: false,
                    processData: false,
        ...
        $("#addBannerForm").validate({
            ...
            submitHandler: function(form) {
                $.ajax({
                    url: '{{ route("public.notice.banner.store") }}',
                    method: 'POST',
                    data: new FormData(form),
                    contentType: false,
                    processData: false,
        ...
        function fetchBanners() {
            $.ajax({
                url: '{{ route("public.notice.banner.get") }}', // Replace with your route if necessary
                method: 'GET',
                success: function(response) {
                    if (response.success) {
