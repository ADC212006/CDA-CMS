                    ...
                    <div class="col-12">
                        ...
                        <!-- <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Public Notes
                                </li>
                            </ol>
                        </div> -->
                    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addBannerForm" class="row gy-4 gx-3" enctype="multipart/form-data">
                    ...
                    <div class="col-xl-6 mt-2">
                        <label class="form-label" for="addFirstLayer">1st Layer</label>
                        <textarea class="form-control" id="addFirstLayer" name="addFirstLayer"
                            placeholder="Enter First Layer"></textarea>
                    </div>
                    <div class="col-xl-6 mt-2">
                        <label class="form-label" for="addDescription">Add Description</label>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-transparent">
                <h5 class="modal-title" id="addModalLabel">Add New Auction Notes</h5>
...
<script>
    $(document).ready(function() {
        var table = $('.datatables-ajax').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("auction.notices.fetch") }}', // Update the API endpoint
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        return `
                <p id="action-${data}">
                 <a href="{{ env('WEBSITE_URL') . 'auctionNotices' }}" target="_blank">
        ...
        $("#editForm").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: '{{ route("auction.notices.update", ":id") }}'.replace(':id', id),
                    method: 'POST',
        ...
        var deleteUrl = "{{ route('auction.notices.destroy', ['id' => '__id__']) }}";
        ...
        $("#addForm").validate({
            ...
            submitHandler: function(form) {
                // On form validation success, make an AJAX request
                $.ajax({
                    url: '{{route("auction.notices.store")}}', // Your API endpoint
                    method: 'POST',
                    data: new FormData(form),
                    contentType: false,
                    processData: false,
        ...
        $("#addBannerForm").validate({
            ...
            submitHandler: function(form) {
                $.ajax({
                    url: '{{ route("auction.notices.banner.store") }}',
                    method: 'POST',
                    data: new FormData(form),
                    contentType: false,
                    processData: false,
                    success: function() {
                        Swal.fire({
                            title: 'Success!',
                            text: 'Auction notice banner created successfully.',
                            icon: 'success',
        ...
        function fetchBanners() {
            $.ajax({
                url: '{{ route("auction.notices.banner.get") }}', // Replace with your route if necessary
                method: 'GET',
                success: function(response) {
                    if (response.success) {
