        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addBannerForm" class="row gy-4 gx-3" enctype="multipart/form-data">
                    ...
                    <div class="col-xl-6 mt-2">
                        <label class="form-label" for="firstLayer">1st Layer</label>
                        <input class="form-control" id="firstLayer" name="firstLayer"
                            placeholder="Enter firstLayer">                   
                    </div>
                    <div class="col-xl-6 mt-2">
...
<script>
    $(document).ready(function() {
        var table = $('.datatables-ajax').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("tender.fetch") }}', // Update the API endpoint
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        return `
                <p id="action-${data}">
       <a href="{{ env('WEBSITE_URL') . 'tenders#tenders' }}" target="_blank">
    <i data-feather="eye" style="width: 20px; height: 20px; color: #6e6b7b;"></i>
        ...
        $("#editForm").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: '{{ route("tender.update", ":id") }}'.replace(':id', id),
                    method: 'POST',
        ...
        var deleteUrl = "{{ route('tender.destroy', ['id' => '__id__']) }}";
        ...
        $("#addForm").validate({
            ...
            submitHandler: function(form) {
                // On form validation success, make an AJAX request
                $.ajax({
                    url: '{{route("tender.store")}}', // Your API endpoint
                    method: 'POST',
                    data: new FormData(form),
                    contentType: false,
                    processData: false,
        ...
        function fetchBanners() {
            $.ajax({
                url: '{{ route("tender.banner.get") }}', // Replace with your route if necessary
                method: 'GET',
                success: function(response) {
                    if (response.success) {
                        ...
                        $.each(banners, function(index, banner) {
                            ...
                            <tr>
                                ...
                                <td>${banner.status}</td>
                                <td> 
                                 <a href="{{ env('WEBSITE_URL') . 'tenders' }}" target="_blank">
                                    <i data-feather="eye" style="width: 20px; height: 20px; color: #6e6b7b  ;"></i>
                                </a>         
